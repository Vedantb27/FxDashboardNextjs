...
interface SpotPendingModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function SpotPendingModal({
  ...
  useEffect(() => {
    if (!isOpen) return;
    const parent = pending.find((p) => p.id === currentAction.parentId);
    if (!parent) {
      toast.error("Parent pending order not found");
      onClose();
      return;
    }
  ...
  const validate = () => {
    ...
    if (form.entry_price <= 0) {
      ...
      if (!e.entry_price && parent.price) {
        const parentPrice = parent.price;

        if (parent.trade_setup === "buy" && form.entry_price <= parentPrice) {
          e.entry_price = `Entry must be ABOVE parent price (${parentPrice})`;
        }

        if (parent.trade_setup === "sell" && form.entry_price >= parentPrice) {
          e.entry_price = `Entry must be BELOW parent price (${parentPrice})`;
        }
      }
    }
    ...
    if (form.risk_percentage < 0 || form.risk_percentage > 100)
